...
# Thêm thành viên mới
from django.contrib.auth.models import User, Group  # Thêm import Group
...
def them_thanh_vien(request):
    if request.method == 'POST':
        ...
        if vai_tro == 'quan_ly':
            quan_ly_group = Group.objects.get(name='quanly')  # Lấy nhóm quanly
            user.groups.add(quan_ly_group)  # Thêm user vào nhóm
        else:
            nhan_vien_group = Group.objects.get(name='nhanvien')  # Lấy nhóm nhanvien
            user.groups.add(nhan_vien_group)  # Thêm user vào nhóm
...
def sua_thanh_vien(request, user_id):
    ...
    if request.method == 'POST':
        ...
        # Cập nhật nhóm
        thanh_vien.groups.clear()  # Xóa tất cả nhóm hiện tại
        if vai_tro == 'quan_ly':
            quan_ly_group = Group.objects.get(name='quanly')
            thanh_vien.groups.add(quan_ly_group)
        else:
            nhan_vien_group = Group.objects.get(name='nhanvien')
            thanh_vien.groups.add(nhan_vien_group)
